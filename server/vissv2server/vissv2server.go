...
/*	router := mux.NewRouter()
	...
	for _, serverComponent := range serverComponents {
		...
		case "ddsMgr":
			if *ddsEnable {
				// DDS channels must be unbuffered: getVehicleTopic performs a
				// synchronous VIN request/response handshake across the split
				// request/response channels (PR #187).
				if *ddsDomain != 0 {
					ddsMgr.SetDomain(dds.Domain(*ddsDomain))
				}
				transportMgrChannel[5] = make(chan string)
				toTransportChannel[5] = make(chan string)
				go ddsMgr.DdsMgrInit(5, transportMgrChannel[5], toTransportChannel[5])
